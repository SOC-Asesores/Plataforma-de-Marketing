      ...
      <div class="row">
        <div class="col-12">
          <nav class="navbar navbar-expand-lg navbar-light bg-light">
            ...
            <form class="form-inline" method="post" action="{{ route('searchFiles') }}">
              @csrf
      ...
      <div class="row">
        <div class="col-12">
          <ul class="d-inline-block" style="list-style: none;">
            <li class="d-inline-block mr-4"><a class="btn btn-primary" href="{{ route("dashboard") }}">Archivos</a></li>
            <li class="d-inline-block mr-4"><a href="{{ route("dashboard2") }}">Carpetas</a></li>
            <li class="d-inline-block mr-4"><a href="{{ route("dashboard3") }}">Botones</a></li>
          </ul>
        </div>
      </div>
      <div class="row">
          ...
          <table class="table">
            ...
            <tbody>
             @foreach($archivos as $archivo)
              @php
                $validar_archivo = checkFolders($archivo->categoria);
              @endphp
              @if($validar_archivo != null)
                <tr>
                  <th scope="row">{{ $archivo->count }}</th>
                  <td>{{ $archivo->nombre }}</td>
                  <td><a target="_blank" href="http://marketing.socasesores.com/categoria/{{ $archivo->categoria }}">{{ $archivo->categoria }}</a></td>
                </tr>
              @endif
             @endforeach
            </tbody>
